...
def build_lag_analyze_row() -> RowSpec:
    row_builder = row('Lag analyze', default_height=4, default_span=12)
    ...
    eventfeed_error_m.add_query(
        expr_sum_increase(
            'ticdc_kvclient_event_feed_error_count',
            by_labels=['type'],
            scope='instance',
        ),
        legend='{{type}}',
    ...
    row_builder.add_graph(eventfeed_error_m)
    ...
    row_builder.add_graph(pd_operator_m)

    tidb_query_duration = graph(
        'TiDB Query Duration',
        description='99.9% of TiDB query durations.',
        unit='s',
        min='0',
    )

    tidb_query_duration.add_query(
        expr_histogram_quantile(
            0.999,
    ...
    row_builder.add_graph(tidb_query_duration)

    tikv_min_resolved_ts_lag = graph(
    ...
    tikv_min_resolved_ts_lag.add_query(
    ...
    row_builder.add_graph(tikv_min_resolved_ts_lag)
    ...
    row_builder.add_graph(sink_write_rows_s)
    ...
    row_builder.add_graph(sink_write_duration)
    ...
    row_builder.add_graph(tikv_scan_tasks_m)
    ...
    tikv_scan_region_time_m.add_query(
        expr_sum_increase(
            'tikv_cdc_scan_duration_seconds_sum',
            by_labels=['instance'],
            scope='tikv_instance',
    ...
    row_builder.add_graph(tikv_scan_region_time_m)
    ...
    row_builder.add_graph(tikv_leader_change)
    ...
    row_builder.add_graph(tikv_admin_apply_s)
    ...
    tikv_advance_resolved_ts_s.add_query(
        f'{expr_sum_rate('tikv_cdc_resolved_ts_gap_seconds_count', by_labels=['instance'], scope='tikv_instance')} / {expr_sum('tikv_cdc_region_resolve_status', by_labels=['instance'], scope='tikv_instance', selectors=[eq('status', 'resolved')])}',
        legend='{{instance}}',
        format='time_series',
    )

    row_builder.add_graph(tikv_advance_resolved_ts_s)
    ...
    tikv_unresolved_region_count.add_query(
        expr_sum(
    ...
    row_builder.add_graph(tikv_unresolved_region_count)
    ...
    tikv_check_leader_region_count_percentile.add_query(
        expr_histogram_quantile(
            0.9999,
    ...
    row_builder.add_graph(tikv_check_leader_region_count_percentile)
    ...
    tikv_advance_resolved_ts_fail_m.add_query(
        expr_sum_increase(
            'tikv_resolved_ts_fail_advance_count',
            by_labels=['reason', 'instance'],
            scope='tikv_instance',
        ),
        legend='{{reason}}-{{instance}}',
        format='time_series',
    )

    row_builder.add_graph(tikv_advance_resolved_ts_fail_m)

    tikv_check_leader_duration_percentile = graph(
    ...
    tikv_check_leader_duration_percentile.add_query(
        expr_histogram_quantile(
            0.9999,
    ...
    row_builder.add_graph(tikv_check_leader_duration_percentile)
    ...
    tikv_cdc_incremental_scan_long_duration_region_count.add_query(
